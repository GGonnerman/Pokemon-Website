...
function correctTextSize() {
	...
	const titleSize = get("title").offsetHeight;
	const subtextSize = get("shorthand").offsetHeight;
	console.log("Sub", subtextSize, "title", titleSize)
	descriptionDiv.style.height = `calc(100% - 2em - 2em - ${titleSize}px - ${subtextSize}px)`;
	// height: calc(100% - 1.8rem - 3.0rem - 2em - 2em);

	let fullHeight = descriptionDiv.clientHeight * .85;
...
function p(prop) {
	return pokedex[Math.floor(Math.random() * 808) + 1][prop]
}

function as(prop) {
	return pokedex[Math.floor(Math.random() * 647) + 1][prop]
}
...
function generateTypeColors() {
	...
	for (let i = 0; i < pokemon.type.length; i++) {
		...
		types.innerHTML += ", ";
		if(document.getElementById("statsTableWrapper").offsetWidth < 450) types.innerHTML += "<br id='small'/>"
	}
}
...
function createEvolutionTable() {
	...
	if (pokemon.evolutions.pre != "" || pokemon.evolutions.post.length > 0) {
		let cell = createLinkedCell(pokemon.evolutions.curr, "currEvolution");
		evolutions.append(cell);
	}
...
function createLinkedCell(id, className) {
	...
	link.href = `/beta/pokemon-page/pokemon.html?pokemon=${id}`;
...
function createBackgroundIcons() {
	...
	for (let i = 0; i < locations.length; i++) {
		...
		let link = document.createElement("a");
		link.href = "/beta/pokemon-page/pokemon.html?pokemon=random";
...
function setAnimatedSpriteLocation() {
	...
	sprite.style.left = `calc(11.5em + ${text.offsetWidth}px + 1.0em)`;
}
...
function createStatsCanvas() {
	...
	for (let i = 0; i < entries.length; i++) {
		...
		if(canvas.offsetWidth < 700) translatedKey = translation[key];
		let value = entries[i][1];
		let currWidth = (2 * i + 1) *
			(width / 13);
		let currHeight = Math.min(height - ((value / max) * halfHeight), height - 25);
		ctx.beginPath();
		ctx.fillStyle = 'black';
		ctx.fillText(value, (currWidth + width / 26) - (value.toString().length * 5), Math.min(currHeight - 4, height - 40));
		ctx.fillText(translatedKey, (currWidth + width / 26) - (translatedKey.toString().length * 5), height - 6);
		ctx.stroke();
...
function randomEverything() {
	window.location.href = "/beta/pokemon-page/pokemon.html?pokemon=random";
}
